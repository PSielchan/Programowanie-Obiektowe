...
public class GrassFieldTest {
    ...
    @Test
    public void only2bushes(){
        ArrayList<MoveDirection> directions = OptionsParser.parse(new String[]{"f","f","f","f","f"});
        ArrayList<Vector2d> positions = new ArrayList<>();
        positions.add(new Vector2d(2,0));
        positions.add(new Vector2d(2,0));
        WorldMap mapa = new GrassField(2);
        Simulation simulation = new Simulation(positions, directions, mapa);
        System.out.println(mapa.toString());
        simulation.run();
        assert mapa.isOccupied(new Vector2d(2, 5));
    }
    @Test
